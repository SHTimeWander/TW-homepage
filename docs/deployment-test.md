...
1. **构建**：使用 Dockerfile 构建应用，将镜像推送至 GHCR，标签为 `main` 和提交 SHA。
2. **部署**：通过 SSH 跳板机上传 Compose 配置，测试服务器拉取对应 SHA 的镜像并执行 `docker compose up -d --wait`。
...
- Secrets：`SSH_JUMP_PASSWORD`、`SSH_TEST_PASSWORD`。
...
| Variable                 | 值                                                |
| ------------------------ | ------------------------------------------------- |
| `APP_IMAGE`              | `ghcr.io/shtimewander/tw-homepage`                |
...
可选仓库 Secret `APP_ENV_FILE` 保存应用环境变量，一行一个 `KEY=value`。未配置时使用空文件。GHCR 使用工作流自带的 `GITHUB_TOKEN` 登录，无需另建镜像仓库密码；服务器登录凭据在部署结束后清理。

测试服务器需要启用 Docker Engine 和 Docker Compose，并能访问 GHCR。应用配置保存在 `/opt/tw-homepage/config`，升级时保留。容器使用 UID/GID 1000，卷带 `:Z` 以支持 SELinux。
